...
There are `r nrow(accidents)` observations in the dataset


2.  Run `View(accidents)` in your Console to view the data in the data viewer. What does each row in the dataset represent?

Each row in the dataset represents a set of information about a distinct accident. Some variables in each row include longitude, latitude, severity, and casualities.
...
During weekdays, accidents tend to occur between 8:00 am and 6:00 pm, whereas during the weekends, accidents tend to occur between noon and 6 pm. In other words, accidents tend to occur earlier (in the morning) during the weekdays. I notice that in this dataset, on the weekdays there are more fatal accidents than the weekends, which do not have any observations of fatal accidents.
...
I notice that most "serious" accidents occur when the road condition is either dry or wet or damp. I also notice that most "serious" accidents occur when the weather is either "Fine + high winds" or "Snowing + No high winds". Outside of these conditions it seems that most car accidents have slight severity. I also notice that there are a lot of empty tiles.
