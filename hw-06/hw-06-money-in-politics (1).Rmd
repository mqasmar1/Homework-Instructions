...
I observe that the country_parent column has been reorganized into distinct country and parent columns. In the country column I see the country where the PAC donation originated as a char variable (categorical), and in the parent column I see the names of parent companies as a char variable (categorical).
...
```{r clean-currency, eval = TRUE}
pac_all <- pac_all %>%
  mutate(
    total  = total  %>% str_remove("\\$") %>% str_remove_all(",") %>% as.numeric(),
    dems   = dems   %>% str_remove("\\$") %>% str_remove_all(",") %>% as.numeric(),
    repubs = repubs %>% str_remove("\\$") %>% str_remove_all(",") %>% as.numeric()
  )
...
```{r canada-mexico-plot, eval=TRUE}
pac_all %>%
    filter(country %in% c("Canada", "Mexico")) %>%
    group_by(country, year) %>%
    summarize(total_contribution = sum(total)) %>%
    ggplot(aes(x = year, y = total_contribution, color = country)) +
    geom_line() +
    labs(
      title = "Total Contributions of PACs from Canada and Mexico per year",
      x = "Year",
...
I observe that Canada consistently contributes more to American PACs than Mexico does. I also notice that generally whenever Canada increases its PAC contributions, Mexico does the same, and when Canada decreases its PAC contributions, Mexico also does. I notice that there was the greatest net increase in PAC contributions between 2007 and 2017 and the greatest net decrease from around 2016 to the present.
...
In this code chunk, the pivot_longer() function merges the Democrat and Republican variables into 1 single categorical variable called "party". This is important for the visualization because the ggplot2 library requires that color be a single variable, and it makes the dataset more tidy.

b.  What would happen if you removed the `group = party` argument from `geom_line()`?

If you removed the `group = party` argument from the `geom_line()` function, ggplot2 would not connect the points lining up with their respective party with a trendline.
...
```{r eval = TRUE, message = FALSE, fig.fullwidth = TRUE, fig.asp = 0.5}
pac_all <- read_csv("data/pac-all.csv")
...
pac_all %>%
  ...
  labs(
    ...
    y = "Amount",
    color = "Party",
    title = "Contribution to US politics from Canada-Connected PACs",
    subtitle = "By party, over time"
  ) +
  theme_minimal()
```
...
UK companies tend to contribute more to the American Republican party instead of the Democratic party. However, the only time between the years 2000 and 2020 where this was not the case was between approximately 2008 and 2011, where more money was donated to the Democratic Party. After 2020, however, the UK contributions to both the Democratic and Republicann parties have dropped.
